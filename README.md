...
<script>
    ...
    // 6. Render History (เฉพาะเมื่อครูล็อกอิน)
    function renderHistory() {
        if (!records.length) {
            historyList.innerHTML = '<div style="text-align: center; color: var(--text-sub); padding: 20px;">ยังไม่มีข้อมูลการเช็คชื่อ</div>';
            return;
        }

        historyList.innerHTML = records.map(rec => {
            const isLate = rec.status === 'สาย';
            const badgeClass = isLate ? 'late' : 'normal';
            const defaultImg = 'data:image/svg+xml;utf8,<svg xmlns="http://www.w3.org/2000/svg" width="50" height="50" viewBox="0 0 24 24" fill="none" stroke="%2394a3b8" stroke-width="2"><path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"></path><circle cx="12" cy="7" r="4"></circle></svg>';
            
            return `
                <div class="history-item">
                    <img src="${rec.photo || defaultImg}" class="history-thumb" alt="รูปโปรไฟล์">
                    <div style="flex: 1;">
                        <div style="font-weight: bold; font-size: 0.95rem;">
                            ${rec.name} (${rec.nickname || '-'}) 
                            <span style="font-weight: normal; font-size: 0.8rem; color: var(--text-sub);">
    ...
    exportCsvBtn.addEventListener('click', () => {
        ...
        csvContent += "วันที่,รหัสนักเรียน,ชื่อ-นามสกุล,ชื่อเล่น,ชั้น,ห้อง,เวลา,สถานะ,พิกัด GPS\n";

        records.forEach(r => {
            csvContent += `"${r.checkDate}","${r.studentId}","${r.name}","${r.nickname}","${r.grade}","${r.room}","${r.time}","${r.status}","${r.gps}"\n`;
        });
